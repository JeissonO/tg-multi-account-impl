variable "name" {
  description = "Name of the VPC. If not provided, will use 'jo-vpc-{region}'"
  type        = string
  default     = null
}

variable "cidr" {
  description = "Base CIDR block for the VPC. Must be a /16 CIDR block"
  type        = string

  validation {
    condition     = can(regex("^([0-9]{1,3}\\.){3}[0-9]{1,3}/16$", var.cidr))
    error_message = "The cidr value must be a valid CIDR block with /16 mask (e.g., 10.0.0.0/16)"
  }
}
...
variable "create_igw" {
  description = "Controls if an Internet Gateway is created for public subnets. Defaults to false as outbound traffic is managed through AWS Cloud WAN. Set to true only if direct internet access is required for specific use cases."
  type        = bool
  default     = false
}
...
variable "vpc_flow_log_iam_policy_name" {
  ...
  type        = string
  default     = "VPCFlowLogToCloudWatch"
}
